...
final class Check {
	...
	protected static void equalSizes(float[][] A, float[][] B){
		if(A.length!=B.length ||A[0].length!=B.length) throw new IllegalArgumentException("Matrix dimensions must agree.");
	}
	...
	protected static void size0(int size){
		if(size<3) throw new IllegalArgumentException("Empty input array.");
	}
	...
	protected static void size20(int size){
		if(size<3) throw new IllegalArgumentException("Input size needs to be greater than twenty.");
	}
